...
class AcquisitionOptimizer(object):
    ...
    def optimize_fix(self, condtion, f=None, df=None, f_df=None, duplicate_manager=None):
        """
        Optimizes the input function.
        ...
        self.df = df
        self.f_df = f_df
        fixbounds = self.space.get_bounds()[:]
        for i in range(len(condtion)):
            fixbounds[i] = (condtion[i], condtion[i])

        ## --- Update the optimizer, in case context has beee passed.
        self.optimizer = choose_optimizer(self.optimizer_name, fixbounds)
